...
private let BDDefaultFadeSize: CGFloat = 30.0
private let BDDefaultFadeDuration: CFTimeInterval = 0.3

@IBDesignable
class BDFadingScrollView: UIScrollView {
    ...
    convenience init() {
        self.init(fadeSize: BDDefaultFadeSize)
    }
...
// MARK: - Setup helpers
private extension BDFadingScrollView {
    func setDefaults() {
        fadeAxis = .vertical
        fadeLeadingEdge = true
        fadeTrailingEdge = true
        fadeSize = BDDefaultFadeSize
        fadeDuration = BDDefaultFadeDuration
        maskScrollBars = true
    }
